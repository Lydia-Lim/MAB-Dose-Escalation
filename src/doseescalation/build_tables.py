...
def _opt_col_body_style(opt_col):
    # Dark green + bold on the optimal-dose column body cells (on-screen display
    # only; export_table2_latex uses its own lighter, LaTeX-safe green):
    def col_style(s):
        on = s.name[1] == opt_col
        return ['background-color: #2ca02c82; font-weight: bold' if on else '' for _ in s]
    return col_style
